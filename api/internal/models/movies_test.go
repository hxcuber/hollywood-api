...
func testMoviesExists(t *testing.T) {
	...
	e, err := MovieExists(ctx, tx, o.MovieID)
	if err != nil {
...
func testMoviesFind(t *testing.T) {
	...
	movieFound, err := FindMovie(ctx, tx, o.MovieID)
	if err != nil {
		t.Error(err)
	}
...
func testMovieToOneActorUsingMainActor(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, movieDBTypes, false, movieColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Movie struct: %s", err)
	}
	...
	local.MainActorID = foreign.ActorID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.MainActor().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	AddActorHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Actor) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := MovieSlice{&local}
	if err = local.L.LoadMainActor(ctx, tx, false, (*[]*Movie)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.MainActor == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.MainActor = nil
	if err = local.L.LoadMainActor(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.MainActor == nil {
		t.Error("struct should have been eager loaded")
...
func testMovieToOneSetOpActorUsingMainActor(t *testing.T) {
	var err error
	...
	for i, x := range []*Actor{&b, &c} {
		err = a.SetMainActor(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if x.R.MainActorMovies[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.MainActorID != x.ActorID {
			t.Error("foreign key was wrong value", a.MainActorID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.MainActorID))
		reflect.Indirect(reflect.ValueOf(&a.MainActorID)).Set(zero)
		...
		if a.MainActorID != x.ActorID {
			t.Error("foreign key was wrong value", a.MainActorID, x.ActorID)
		}
	}
}
...
var (
	movieDBTypes = map[string]string{`MovieID`: `bigint`, `Name`: `character varying`, `MainActorID`: `bigint`, `ReleaseDate`: `date`}
	_            = bytes.MinRead
)
