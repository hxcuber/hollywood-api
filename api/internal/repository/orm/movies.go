...
var MovieTableColumns = struct {
	...
	Name:        "movies.name",
	MainActorID: "movies.main_actor_id",
...
var MovieWhere = struct {
	...
	MainActorID whereHelperint64
	ReleaseDate whereHelpertime_Time
}{
	MovieID:     whereHelperint64{field: "\"movies\".\"movie_id\""},
	Name:        whereHelperstring{field: "\"movies\".\"name\""},
	MainActorID: whereHelperint64{field: "\"movies\".\"main_actor_id\""},
...
type movieR struct {
	MainActor *Actor `boil:"MainActor" json:"MainActor" toml:"MainActor" yaml:"MainActor"`
}
...
func (r *movieR) GetMainActor() *Actor {
	if r == nil {
		return nil
	}
	return r.MainActor
...
var (
	movieAllColumns            = []string{"movie_id", "name", "main_actor_id", "release_date"}
	movieColumnsWithoutDefault = []string{"name", "main_actor_id", "release_date"}
	movieColumnsWithDefault    = []string{"movie_id"}
	moviePrimaryKeyColumns     = []string{"movie_id"}
...
func (q movieQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Movie, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "orm: failed to execute a one query for movies")
	}
...
func (q movieQuery) All(ctx context.Context, exec boil.ContextExecutor) (MovieSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "orm: failed to assign all query results to Movie slice")
	}
...
func (q movieQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "orm: failed to count movies rows")
	}
...
func (q movieQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "orm: failed to check if movies exists")
	}

	return count > 0, nil
}

// MainActor pointed to by the foreign key.
func (o *Movie) MainActor(mods ...qm.QueryMod) actorQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"actor_id\" = ?", o.MainActorID),
	}
...
// LoadMainActor allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (movieL) LoadMainActor(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMovie interface{}, mods queries.Applicator) error {
	var slice []*Movie
	var object *Movie
	...
	if singular {
		...
		args = append(args, object.MainActorID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.MainActorID {
					continue Outer
				}
			}

			args = append(args, obj.MainActorID)
	...
	query := NewQuery(
		qm.From(`actors`),
		qm.WhereIn(`actors.actor_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for actors")
	}
	...
	if singular {
		...
		object.R.MainActor = foreign
		if foreign.R == nil {
			foreign.R = &actorR{}
		}
		foreign.R.MainActorMovies = append(foreign.R.MainActorMovies, object)
		return nil
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.MainActorID == foreign.ActorID {
				local.R.MainActor = foreign
				if foreign.R == nil {
					foreign.R = &actorR{}
				}
				foreign.R.MainActorMovies = append(foreign.R.MainActorMovies, local)
				break
			}
		}
	}
...
func (o *Movie) SetMainActor(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Actor) error {
	...
	values := []interface{}{related.ActorID, o.MovieID}
	...
	o.MainActorID = related.ActorID
	if o.R == nil {
		o.R = &movieR{
			MainActor: related,
		}
	} else {
		o.R.MainActor = related
	}

	if related.R == nil {
		related.R = &actorR{
			MainActorMovies: MovieSlice{o},
		}
	} else {
		related.R.MainActorMovies = append(related.R.MainActorMovies, o)
	}
...
func FindMovie(ctx context.Context, exec boil.ContextExecutor, movieID int64, selectCols ...string) (*Movie, error) {
	movieObj := &Movie{}
	...
	query := fmt.Sprintf(
		"select %s from \"movies\" where \"movie_id\"=$1", sel,
	)

	q := queries.Raw(query, movieID)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "orm: unable to select from movies")
	}
...
func (o *Movie) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("orm: no movies provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "orm: unable to insert into movies")
	}
...
func (o *Movie) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("orm: unable to update movies, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "orm: unable to update movies row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "orm: failed to get rows affected by update for movies")
	}
...
func (q movieQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "orm: unable to update all for movies")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "orm: unable to retrieve rows affected for movies")
	}
...
func (o *Movie) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("orm: no movies provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("orm: unable to upsert movies, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "orm: unable to upsert movies")
	}
...
func (o *Movie) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("orm: no Movie provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "orm: unable to delete from movies")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "orm: failed to get rows affected by delete for movies")
	}
...
func (q movieQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("orm: no movieQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "orm: unable to delete all from movies")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "orm: failed to get rows affected by deleteall for movies")
	}
...
func (o *Movie) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindMovie(ctx, exec, o.MovieID)
	if err != nil {
		return err
	}
...
func (o *MovieSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "orm: unable to reload all in MovieSlice")
	}
...
func MovieExists(ctx context.Context, exec boil.ContextExecutor, movieID int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"movies\" where \"movie_id\"=$1 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, movieID)
	}
	row := exec.QueryRowContext(ctx, sql, movieID)

	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "orm: unable to check if movies exists")
	}
...
func (o *Movie) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return MovieExists(ctx, exec, o.MovieID)
}
